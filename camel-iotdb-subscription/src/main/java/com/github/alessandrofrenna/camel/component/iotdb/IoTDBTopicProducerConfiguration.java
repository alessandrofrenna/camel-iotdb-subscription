...
/**
 * The <b>IoTDBTopicProducerConfiguration</b> class keeps track of the uri parameters used by the
 * producer.
 */
@UriParams
public class IoTDBTopicProducerConfiguration {
  ...
  /**
   * Get the action passed as route uri parameter.</br> The value of action is required. It must be
   * one of "create" or "delete".</br> The "create" action will be used to create a topic at a given
   * by {@link #getPath()}.</br> The "delete" action will delete the topic.
   *
  ...
  /**
   * Get the timeseries path passed as route uri parameter.</br> The value of the path is required
   * only for the "create" action. When action is "delete" this parameter, if passed is ignored.
   *
