...
/**
 * The <b>IoTDBTopicConsumer</b> extends the camel {@link DefaultConsumer}. </br> It is used to
 * create a {@link SubscriptionPushConsumer} used to get message from an IoTDB topic after
 * subscription
 */
public class IoTDBTopicConsumer extends DefaultConsumer {
  ...
  private ConsumeListener getConsumeListener() {
    return (message) -> {
      ...
      exchange.getIn().setHeader(TOPIC_HEADER_KEY, message.getCommitContext().getTopicName());
