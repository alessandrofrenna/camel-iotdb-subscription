...
public class IoTDBTopicActionsTests extends IoTDBTestSupport {
    ...
    //        final String topicName = "rain_topic1";
    //        final String body =
    //                String.format("iotdb-subscription:%s?action=create&path=root.test.demo_device.rain", topicName);
    //
    //        doInSession((session) -> assertTrue(session.getTopic("rain_topic").isEmpty()));
    //        producerTemplate.sendBody(body, null);
    //
    //        Awaitility.await()
    //                .atMost(Duration.ofSeconds(1))
    //                .untilAsserted(() -> doInSession(session -> {
    ...
    //                    assertTrue(
    //                            session.getTopic(topicName).isPresent(),
    //                            String.format("topic with name '%s' found", topicName));
    //                }));
    //    }
    //
    //    @Test
    //    public void drop_rain_topic_from_iotdb() {
    //        producerTemplate.sendBody("iotdb-subscription:rain_topic?action=create&path=root.test.demo_device.rain",
    // null);
    //
    //        doInSession((session) ->
    ...
    //                    assertTrue(session.getTopic("rain_topic").isEmpty(), "topic with name 'rain_topic' not
    // found");
    //                }));
    //    }
    //
    ...
    //                (mock, ctx) -> doThrow(new RuntimeException("")).when(mock).open())) {
    //            producerTemplate.sendBody("iotdb-subscription:rain_topic?action=drop", null);
    //        }
    //
    //        assertEquals(2, publishedEvents.size());
